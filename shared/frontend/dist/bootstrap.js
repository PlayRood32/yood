...
function removeBootstrapAds(root = document) {
    const renderers = [];
    if (root instanceof Element && root.matches(BOOTSTRAP_AD_SELECTOR))
        renderers.push(root);
...
function installBootstrapAdShield() {
    const install = () => {
...
(() => {
    ...
    const install = () => {
        ...
        if (!documentRoot || document.getElementById("yood-boot"))
            return;
        const style = document.createElement("style");
    ...
    `;
      ...
      <button id="yood-boot-retry" type="button">Retry</button>
        ...
        const timeout = window.setTimeout(() => {
            if (finished)
                return;
            const message = overlay.querySelector("#yood-boot-message");
            const button = overlay.querySelector("#yood-boot-retry");
            if (message)
                message.textContent = "YouTube did not finish loading. Check your network and WebKit/GStreamer packages.";
            if (button)
        ...
        overlay.querySelector("#yood-boot-retry")?.addEventListener("click", () => window.location.reload());
