...
const playerStatus = document.querySelector<HTMLElement>("#status")!;
const stats = document.querySelector<HTMLElement>("#stats")!;
const speed = document.querySelector<HTMLSelectElement>("#speed")!;
const pictureInPicture = document.querySelector<HTMLButtonElement>("#pip")!;
...
async function load(payload: unknown): Promise<void> {
  const data = payload as { url?: string; title?: string };
  if (!data.url) return;
  video.src = data.url;
  title.textContent = data.title || "Yood Native Player";
  playerStatus.textContent = "Ready";
  try { await video.play(); } catch { playerStatus.textContent = "Ready — press play"; }
}

const tauri = (window as unknown as { __TAURI__?: { core?: { invoke?: (command: string, args?: Record<string, unknown>) => Promise<unknown> }, event?: { listen?: (event: string, callback: (event: { payload: unknown }) => void) => Promise<() => void> } } }).__TAURI__;
...
void (async () => {
  ...
  const payload = await tauri?.core?.invoke?.("take_player_source") as { source?: string; title?: string } | null | undefined;
  if (payload?.source) {
...
pictureInPicture.addEventListener("click", async () => {
  const player = video as ExtendedVideo;
  try {
...
window.addEventListener("keydown", (event) => {
  ...
  if (target && ["INPUT", "SELECT", "TEXTAREA", "BUTTON"].includes(target.tagName)) {
    if (target === timeline && (event.key === "ArrowLeft" || event.key === "ArrowRight")) return;
    return;
  }
