...
type DownloadMode = "video" | "audio" | "songs";
...
interface Window {
  __YOOD__?: { invoke: Invoke; scan: () => void };
  __YOOD_BOOT__?: { complete: () => void };
  __YOOD_RELEASE_AD_SHIELD__?: () => void;
  __TAURI__?: {
    core?: { invoke?: Invoke };
    event?: { listen?: (event: string, callback: TauriEventCallback) => Promise<TauriUnlisten> };
  };
}

const invoke: Invoke = async (command, args) => {
  const fn = window.__TAURI__?.core?.invoke;
  if (!fn) throw new Error("Yood integration is not available");
...
async function listenEvent(event: string, callback: TauriEventCallback): Promise<TauriUnlisten> {
...
let selectedAppearance: Appearance = "system";
let systemAppearance: MediaQueryList | undefined;
let detectedSystemAppearance: "light" | "dark" | undefined;
let appearanceRevision = 0;
let appearanceTimers: number[] = [];
...
const THEME_SETTLE_DELAYS = [160, 1_100];

function effectiveAppearance(appearance: Appearance): "light" | "dark" {
  return appearance === "system"
    ? (detectedSystemAppearance ?? (systemAppearance?.matches ? "dark" : "light"))
    : appearance;
}

function clearAppearanceTimers(): void {
  appearanceTimers.forEach((timer) => window.clearTimeout(timer));
  appearanceTimers = [];
}

function applyEffectiveAppearance(): void {
  ...
  let style = document.getElementById("yood-theme-style") as HTMLStyleElement | null;
...
function stabilizeAppearance(): void {
...
function applyAppearance(appearance: Appearance, userInitiated = false): void {
  if (userInitiated) appearanceRevision += 1;
...
function installAppearance(): void {
  ...
  const reapply = (): void => applyAppearance(selectedAppearance);
...
function ensureRoot(): HTMLElement {
  ...
  if (!document.head || !document.body) return document.documentElement;
  if (!root) {
...
function currentUrl(): string { return window.location.href; }
...
function parseYoutubeUrlInput(raw: string): string | null {
...
function showUrlBar(): void {
  ...
  const closeBar = (): void => bar.remove();
...
function videoUrlFrom(element: Element): string | null {
  const link = element.querySelector<HTMLAnchorElement>('a[href*="/watch?v="], a[href*="/shorts/"], a[href*="youtu.be/"]');
  if (!link) return null;
  try {
...
function entryFromElement(element: Element): VideoEntry | null {
...
function makeButton(url: string, context: string, title = "Download"): HTMLButtonElement {
  ...
  button.addEventListener("click", (event) => {
    ...
    if (context === "playlist" || context === "channel") void showPlaylistDialog(url, context as "playlist" | "channel");
    else showDownloadDialog(url, context);
  });
  return button;
}

function makeNativePlayerButton(url: string, title: string): HTMLButtonElement {
  const button = document.createElement("button");
...
function injectCardButton(card: Element): void {
  if (card.querySelector("[data-yood-download]")) return;
...
function injectVideoButton(): void {
  ...
  if (!/youtube\.com\/(watch|shorts\/|playlist|channel\/)/i.test(url)) return;
  const existing = document.querySelector<HTMLElement>("[data-yood-main-download]");
  if (existing?.dataset.yoodDownload === url && existing.isConnected) return;
  ...
  if (player) player.dataset.yoodMainAction = "true";
  if (host) {
    host.append(button);
    if (player) host.appendChild(player);
    return;
  }
  ...
  if (player) fallback.appendChild(player);
  document.body?.appendChild(fallback);
}

function ensureNavigation(): void {
  if (!document.body || document.getElementById("yood-navigation")) return;
...
function scan(): void {
  ensureRoot();
  if (document.querySelector("ytd-app, ytd-page-manager")) window.__YOOD_BOOT__?.complete();
...
function ensureAuthFallback(): void {
  ...
  open.addEventListener("click", () => {
    open.disabled = true;
    void invoke("open_external", { url: location.href }).catch((error: unknown) => {
...
function needsCardScan(node: Node): boolean {
...
let closeManagerLive: (() => void) | undefined;
...
function showDownloadDialog(url: string, context: string): void {
  closeOverlay();
  ...
  start.addEventListener("click", async () => {
    start.disabled = true;
    try {
      const mode = (card.querySelector("#yood-mode") as HTMLSelectElement).value as DownloadMode;
      await invoke("enqueue_download", { request: {
        url, mode, quality: (card.querySelector("#yood-quality") as HTMLSelectElement).value,
        container: (card.querySelector("#yood-container") as HTMLSelectElement).value,
        format_selector: (card.querySelector("#yood-format") as HTMLInputElement).value || undefined,
        subtitles: (card.querySelector("#yood-subs") as HTMLInputElement).checked,
        embed_thumbnail: (card.querySelector("#yood-thumb") as HTMLInputElement).checked || mode === "songs",
        context,
      }});
      closeOverlay();
      toast("Added to Yood's download queue");
    } catch (error) { toast(`Could not queue download: ${String(error)}`); start.disabled = false; }
  });
  ...
  (card.querySelector("#yood-mode") as HTMLSelectElement).addEventListener("change", (event) => {
    const audio = (event.target as HTMLSelectElement).value !== "video";
    (card.querySelector("#yood-quality") as HTMLSelectElement).disabled = audio;
    (card.querySelector("#yood-container") as HTMLSelectElement).disabled = audio;
  });
}

async function showPlaylistDialog(url: string, kind: "playlist" | "channel"): Promise<void> {
  closeOverlay();
  const overlay = document.createElement("div");
  overlay.className = "yood-overlay";
  const card = document.createElement("section");
  ...
  try {
    const result = await invoke("inspect_url", { url }) as { title?: string; entries?: VideoEntry[] };
    const entries = result.entries || [];
    card.innerHTML = `<h2>${escapeHtml(result.title || (kind === "playlist" ? "Playlist" : "Channel"))}</h2>`;
    const selectActions = document.createElement("div"); selectActions.className = "yood-actions";
    const selectAll = button("Select all"); const selectNone = button("Deselect all");
    const list = document.createElement("div");
    entries.forEach((entry) => {
      const row = document.createElement("div"); row.className = "yood-entry";
    ...
    selectAll.addEventListener("click", () => list.querySelectorAll<HTMLInputElement>("input").forEach((input) => input.checked = true));
    selectNone.addEventListener("click", () => list.querySelectorAll<HTMLInputElement>("input").forEach((input) => input.checked = false));
    selectActions.append(selectAll, selectNone); card.append(selectActions, list);
    const options = document.createElement("div"); options.innerHTML = `<h3>Download options</h3><div class="yood-row"><label>Mode</label><select id="yood-list-mode"><option value="video">Video</option><option value="audio">Audio (MP3)</option><option value="songs">Songs (MP3 + metadata)</option></select></div><div class="yood-row"><label>Quality</label><select id="yood-list-quality"><option value="best">Best available</option><option value="1080">1080p or best</option><option value="720">720p or best</option><option value="480">480p or best</option></select></div>`;
    card.appendChild(options);
    const actions = document.createElement("div"); actions.className = "yood-actions";
    const cancel = button("Cancel"); cancel.addEventListener("click", closeOverlay);
    const start = button("Download selected", true); start.addEventListener("click", async () => {
      const selected = [...list.querySelectorAll<HTMLInputElement>("input:checked")].map((input) => input.dataset.yoodEntry).filter((x): x is string => Boolean(x));
      if (!selected.length) return toast("Select at least one video");
      start.disabled = true;
      try {
        const mode = (card.querySelector("#yood-list-mode") as HTMLSelectElement).value;
        const quality = (card.querySelector("#yood-list-quality") as HTMLSelectElement).value;
        for (const entryUrl of selected) await invoke("enqueue_download", { request: { url: entryUrl, mode, quality, container: "mp4", context: kind } });
        closeOverlay(); toast(`Added ${selected.length} video${selected.length === 1 ? "" : "s"} to the queue`);
      } catch (error) { toast(`Could not queue playlist: ${String(error)}`); start.disabled = false; }
    });
    actions.append(cancel, start); card.appendChild(actions);
  } catch (error) { card.innerHTML = `<h2>Unable to read ${kind}</h2><p>${escapeHtml(String(error))}</p>`; }
}

function ensureLauncher(): void {
  if (!document.body || document.getElementById("yood-launcher")) return;
  const launcher = document.createElement("button");
  launcher.id = "yood-launcher"; launcher.type = "button"; launcher.textContent = "Yood";
  launcher.title = "Open Yood downloads and settings";
  launcher.addEventListener("click", () => showManager());
  document.body.appendChild(launcher);
}

function managerState(status: string): "active" | "queued" | "completed" | "failed" {
  if (status === "downloading" || status === "paused") return "active";
  if (status === "queued" || status === "interrupted") return "queued";
  if (status === "completed") return "completed";
  return "failed";
}

function renderManagerRow(item: DownloadItem): HTMLElement {
  ...
  if (["downloading", "paused", "queued", "interrupted"].includes(item.status)) {
    ...
    action.addEventListener("click", async () => { action.disabled = true; try { await invoke(item.status === "paused" ? "resume_download" : "pause_download", { id: item.id }); } finally { action.disabled = false; } });
    row.appendChild(action);
    const cancel = button("Cancel");
    cancel.addEventListener("click", async () => { cancel.disabled = true; try { await invoke("cancel_download", { id: item.id }); } finally { cancel.disabled = false; } });
    row.appendChild(cancel);
  }
  if (item.status === "failed") {
    const retry = button("Retry");
    retry.addEventListener("click", async () => { retry.disabled = true; try { await invoke("retry_download", { id: item.id }); } finally { retry.disabled = false; } });
  ...
  if (["completed", "failed", "cancelled"].includes(item.status)) {
    ...
    remove.addEventListener("click", async () => {
      ...
      try { await invoke("remove_download", { id: item.id }); row.remove(); }
      catch (error) { toast(`Could not remove download: ${String(error)}`); remove.disabled = false; }
    });
...
async function showManager(): Promise<void> {
  closeOverlay();
  const overlay = document.createElement("div"); overlay.className = "yood-overlay";
  const card = document.createElement("section"); card.className = "yood-card"; card.innerHTML = "<h2>Yood Downloads</h2><p class='yood-muted'>Loading…</p>";
  overlay.appendChild(card); document.body.appendChild(overlay);
  try {
    const items = await invoke("list_downloads") as DownloadItem[];
    card.innerHTML = "<h2>Yood Downloads</h2>";
    const sections: Record<string, HTMLElement> = {};
    for (const state of ["active", "queued", "completed", "failed"]) {
      const heading = document.createElement("h3"); heading.textContent = state[0].toUpperCase() + state.slice(1); card.appendChild(heading);
      sections[state] = document.createElement("div"); card.appendChild(sections[state]);
    }
    items.forEach((item) => sections[managerState(item.status)].appendChild(renderManagerRow(item)));
    const actions = document.createElement("div"); actions.className = "yood-actions";
    const settings = button("Settings"); settings.addEventListener("click", () => showSettings());
    const close = button("Close", true); close.addEventListener("click", closeOverlay); actions.append(settings, close); card.appendChild(actions);
    ...
    const applyUpdate = (item: DownloadItem): void => {
      const existing = sections[managerState(item.status)].querySelector<HTMLElement>(`[data-yood-download-id="${item.id}"]`);
    ...
    const scheduleFullRefresh = (): void => {
      ...
      void invoke("list_downloads").then((freshItems) => {
        ...
        (freshItems as DownloadItem[]).forEach((item) => { seen.add(item.id); applyUpdate(item); });
        card.querySelectorAll<HTMLElement>("[data-yood-download-id]").forEach((row) => {
          if (!seen.has(row.dataset.yoodDownloadId || "")) row.remove();
        });
    ...
    const unlistenPromise = listenEvent("download-updated", (event) => {
      ...
      if (!item?.id || !document.body.contains(card)) return;
  ...
  } catch (error) { card.innerHTML = `<h2>Downloads unavailable</h2><p>${escapeHtml(String(error))}</p>`; }
}

interface YoodSettings {
  appearance: Appearance;
  tracking_protection: boolean;
  aggressive_tracking_protection: boolean;
...
async function showSettings(): Promise<void> {
  closeOverlay();
  const overlay = document.createElement("div"); overlay.className = "yood-overlay";
  const card = document.createElement("section"); card.className = "yood-card"; card.innerHTML = "<h2>Yood Settings</h2><p class='yood-muted'>Loading…</p>"; overlay.appendChild(card); document.body.appendChild(overlay);
  try {
    const [settings, appInfo, accountHints] = await Promise.all([
      invoke("get_settings") as Promise<YoodSettings>,
      invoke("get_app_info").catch(() => null) as Promise<{ version: string; platform: string; binaries: { name: string; version?: string; available: boolean; bundled: boolean }[]; filter: { enabled: boolean; tracking_enabled: boolean; rule_count: number; last_success: string | null } } | null>,
      invoke("get_account_hints").catch(() => null) as Promise<string | null>,
    ]);
    ...
    `;
    (card.querySelector("#yood-theme") as HTMLSelectElement).value = settings.appearance;
    (card.querySelector("#yood-account-label") as HTMLInputElement).value = accountHints || "";
    (card.querySelector("#yood-tracking") as HTMLInputElement).checked = settings.tracking_protection;
    (card.querySelector("#yood-aggressive-tracking") as HTMLInputElement).checked = settings.aggressive_tracking_protection;
    (card.querySelector("#yood-concurrency") as HTMLInputElement).value = String(settings.max_concurrent_downloads);
    (card.querySelector("#yood-folder") as HTMLInputElement).value = settings.download_directory;
    (card.querySelector("#yood-temp") as HTMLInputElement).value = settings.temp_directory;
    (card.querySelector("#yood-clean") as HTMLInputElement).checked = settings.cleanup_temp_files;
    (card.querySelector("#yood-overwrite") as HTMLInputElement).checked = settings.overwrite_existing;
    (card.querySelector("#yood-embed-thumb") as HTMLInputElement).checked = settings.embed_thumbnail;
    (card.querySelector("#yood-subs") as HTMLInputElement).checked = settings.download_subtitles;
    (card.querySelector("#yood-sub-lang") as HTMLInputElement).value = settings.subtitle_language;
    (card.querySelector("#yood-log-level") as HTMLSelectElement).value = settings.log_level;
    (card.querySelector("#yood-shortcut") as HTMLInputElement).value = settings.shortcut_download;

    const updateFilters = card.querySelector<HTMLButtonElement>("#yood-update-filters");
    updateFilters?.addEventListener("click", async () => {
      ...
      try { await invoke("update_filter_lists"); toast("Filter lists updated"); showSettings(); }
      catch (error) { toast(`Could not update filter lists: ${String(error)}`); updateFilters.disabled = false; updateFilters.textContent = original; }
    });
    const updateYtdlp = card.querySelector<HTMLButtonElement>("#yood-update-ytdlp");
    updateYtdlp?.addEventListener("click", async () => {
      ...
      try { const version = await invoke("update_ytdlp"); toast(`yt-dlp updated to ${String(version)}`); showSettings(); }
      catch (error) { toast(`Could not update yt-dlp: ${String(error)}`); updateYtdlp.disabled = false; updateYtdlp.textContent = original; }
    });

    const actions = document.createElement("div"); actions.className = "yood-actions"; const cancel = button("Cancel"); cancel.addEventListener("click", closeOverlay); const save = button("Save", true);
    save.addEventListener("click", async () => {
      save.disabled = true;
      try {
        const appearance = (card.querySelector("#yood-theme") as HTMLSelectElement).value as Appearance;
        const accountLabel = (card.querySelector("#yood-account-label") as HTMLInputElement).value.trim();
        if (accountLabel) await invoke("set_account_hints", { value: accountLabel });
        else await invoke("clear_account_hints").catch(() => undefined);
        await invoke("update_settings", { patch: {
          appearance,
          tracking_protection: (card.querySelector("#yood-tracking") as HTMLInputElement).checked,
          aggressive_tracking_protection: (card.querySelector("#yood-aggressive-tracking") as HTMLInputElement).checked,
          max_concurrent_downloads: Number((card.querySelector("#yood-concurrency") as HTMLInputElement).value),
          download_directory: (card.querySelector("#yood-folder") as HTMLInputElement).value,
          temp_directory: (card.querySelector("#yood-temp") as HTMLInputElement).value,
          cleanup_temp_files: (card.querySelector("#yood-clean") as HTMLInputElement).checked,
          overwrite_existing: (card.querySelector("#yood-overwrite") as HTMLInputElement).checked,
          embed_thumbnail: (card.querySelector("#yood-embed-thumb") as HTMLInputElement).checked,
          download_subtitles: (card.querySelector("#yood-subs") as HTMLInputElement).checked,
          subtitle_language: (card.querySelector("#yood-sub-lang") as HTMLInputElement).value.trim() || "en",
          log_level: (card.querySelector("#yood-log-level") as HTMLSelectElement).value,
          shortcut_download: (card.querySelector("#yood-shortcut") as HTMLInputElement).value.trim().toLowerCase() || "d",
        } });
        applyAppearance(appearance, true);
        downloadShortcut = (card.querySelector("#yood-shortcut") as HTMLInputElement).value.trim().toLowerCase() || "d";
        closeOverlay();
        toast("Settings saved");
      } catch (error) { toast(`Could not save settings: ${String(error)}`); save.disabled = false; }
    });
    actions.append(cancel, save); card.appendChild(actions);
  } catch (error) { card.innerHTML = `<h2>Settings unavailable</h2><p>${escapeHtml(String(error))}</p>`; }
}
...
function containsAdPayload(value: unknown): boolean {
  ...
  for (let inspected = 0; queue.length && inspected < 128; inspected += 1) {
    ...
    for (const [key, nested] of Object.entries(current as Record<string, unknown>)) {
      if (/^(?:ad(?:slot|placements?|slots|break|layout|renderer|playbackcontext)|playerads|promoted(?:sparkles)?(?:renderer)?|companion(?:slot)?renderer)$/i.test(key)) return true;
...
function removeGhostFeedCards(root: ParentNode): boolean {
  const cards: Element[] = [];
  if (root instanceof Element && root.matches(FEED_CARD_SELECTOR)) cards.push(root);
  cards.push(...root.querySelectorAll(FEED_CARD_SELECTOR));
  let removed = false;
  for (const card of cards) {
    if (!card.isConnected || card.querySelector(PLAYABLE_CARD_SELECTOR)) continue;
    const data = (card as unknown as { data?: unknown }).data;
...
function removeKnownAds(root: ParentNode = document): boolean {
  const renderers: Element[] = [];
  if (root instanceof Element && root.matches(AD_RENDERER_SELECTOR)) renderers.push(root);
  ...
  if (removeGhostFeedCards(root)) removed = true;
  return removed;
}
...
function stripPlayerAdPayload(payload: unknown): boolean {
  const queue: unknown[] = [payload];
  const seen = new Set<object>();
  let changed = false;
  for (let inspected = 0; queue.length && inspected < 20_000; inspected += 1) {
    const current = queue.pop();
    if (!current || typeof current !== "object" || seen.has(current)) continue;
    seen.add(current);
    for (const [key, nested] of Object.entries(current as Record<string, unknown>)) {
...
function isPlayerApiRequest(url: string): boolean {
  try {
    const parsed = new URL(url);
...
async function stripPlayerAdsFromResponse(response: Response): Promise<Response> {
  const contentType = response.headers.get("content-type") || "";
  if (!/json/i.test(contentType)) return response;
  try {
    const raw = await response.clone().text();
    if (raw.length > 8 * 1024 * 1024) return response;
    const payload = JSON.parse(raw) as unknown;
    if (!stripPlayerAdPayload(payload)) return response;
...
function installAdAndTrackingLayer(): void {
  if ((window as unknown as Record<string, unknown>).__yoodFilteringInstalled) return;
  (window as unknown as Record<string, unknown>).__yoodFilteringInstalled = true;
  ...
  const trackingTokens: string[] = [];
  const decisionCache = new Map<string, boolean>();
  let cosmeticTimer: number | undefined;
  let cosmeticRequest = 0;
  let lastCosmeticUrl = "";
  void invoke("tracking_protection_enabled").then((enabled) => { if (enabled === true) trackingTokens.push("google-analytics.com", "googletagmanager.com", "youtube.com/api/stats/qoe"); }).catch(() => undefined);
  const requestUrl = (value: unknown): string | null => { try { return new URL(String(value), location.href).href; } catch { return null; } };
  const fastBlocked = (value: unknown): boolean => {
  ...
  const needsEngineCheck = (value: string): boolean => {
    try {
  ...
  const engineBlocked = async (url: string, requestType: string): Promise<boolean> => {
    ...
    try {
      ...
      if (decisionCache.size >= 2048) decisionCache.delete(decisionCache.keys().next().value as string);
  ...
  window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url && fastBlocked(url)) throw new DOMException("Blocked by Yood", "AbortError");
    if (url && needsEngineCheck(url) && await engineBlocked(url, "fetch")) throw new DOMException("Blocked by Yood", "AbortError");
  ...
  window.XMLHttpRequest = class extends OriginalXHR {
    private yoodUrl = "";
    private yoodGeneration = 0;
    private yoodCancelled = false;
    private yoodAsync = true;
    open(method: string, url: string | URL, ...rest: unknown[]): void {
      this.yoodUrl = requestUrl(url) || "";
      this.yoodGeneration += 1;
      this.yoodCancelled = false;
      this.yoodAsync = rest[0] !== false;
      super.open(method, url.toString(), (rest[0] as boolean | undefined) ?? true, rest[1] as string | undefined, rest[2] as string | undefined);
    }
    send(body?: Document | XMLHttpRequestBodyInit | null): void {
      const generation = this.yoodGeneration;
      const url = this.yoodUrl;
      if (!url) { super.send(body); return; }
      if (fastBlocked(url)) { this.abort(); return; }
      if (!this.yoodAsync || !needsEngineCheck(url)) { super.send(body); return; }
      void engineBlocked(url, "xmlhttprequest").then((blocked) => {
        if (generation !== this.yoodGeneration || this.yoodCancelled) return;
        if (blocked) { this.abort(); return; }
        super.send(body);
      });
    }
    abort(): void { this.yoodCancelled = true; super.abort(); }
  } as typeof XMLHttpRequest;
  const updateStyle = (selectors: string[] = []): void => {
    if (!document.head) return;
    let style = document.getElementById("yood-ad-style") as HTMLStyleElement | null;
    if (!style) { style = document.createElement("style"); style.id = "yood-ad-style"; document.head.appendChild(style); }
  ...
  const refreshCosmetics = (): void => {
    ...
    for (const element of Array.from(document.querySelectorAll<HTMLElement>("[class],[id]")).slice(0, 2500)) {
    ...
    void invoke("get_filter_script_rules", { page_url: pageUrl, classes: [...classes], ids: [...ids] }).then((rules) => {
      if (request !== cosmeticRequest || pageUrl !== location.href) return;
      const value = rules as { cosmetic_selectors?: unknown[] };
      const selectors = (value.cosmetic_selectors || []).filter((selector): selector is string => typeof selector === "string" && selector.length <= 2048 && !/[{};]/.test(selector));
      updateStyle(selectors);
      removeKnownAds();
    }).catch(() => updateStyle());
  };
  const scheduleCosmetics = (force = false): void => {
    if (!force && lastCosmeticUrl === location.href) return;
    if (cosmeticTimer === undefined) cosmeticTimer = window.setTimeout(refreshCosmetics, 450);
  };
  const installDomLayer = (): void => {
    updateStyle();
    ...
    new MutationObserver((records) => {
      for (const record of records) {
        for (const node of record.addedNodes) {
          if (node instanceof Element) removeKnownAds(node);
        }
      }
  ...
  if (document.head && document.documentElement) installDomLayer();
  else window.addEventListener("DOMContentLoaded", installDomLayer, { once: true });
  const refreshForNavigation = (): void => { lastCosmeticUrl = ""; scheduleCosmetics(); };
...
function installPlayerAdSkip(): void {
  ...
  const player = (): HTMLElement | null => {
    ...
    return node && node.isConnected ? node as HTMLElement : null;
  };
  const isAdShowing = (node: HTMLElement): boolean =>
    node.classList.contains("ad-showing") || node.classList.contains("ad-interrupting");
  const stopTimer = (): void => {
    if (skipTimer !== undefined) { window.clearInterval(skipTimer); skipTimer = undefined; }
  };
  const restorePlayback = (node: HTMLElement): void => {
    const video = node.querySelector("video.html5-main-video") as HTMLVideoElement | null;
    if (!video) return;
    try {
      if (mutedForAd) { video.muted = false; mutedForAd = false; }
      if (rateForAd) { video.playbackRate = 1; rateForAd = false; }
    } catch { /* transient state; the ad is gone anyway */ }
  };
  const handleAdFrame = (): void => {
    const node = player();
    if (!node || !isAdShowing(node)) { stopTimer(); if (node) restorePlayback(node); return; }
    // Second confirmation from actual ad UI before touching anything.
    const skip = node.querySelector<HTMLElement>(".ytp-skip-ad-button, .ytp-ad-skip-button, .ytp-skip-ad-button-modern");
    const overlay = node.querySelector(".ytp-ad-player-overlay, .ytp-ad-text, .ytp-ad-message-container, .ytp-ad-skip-button-container");
    if (!skip && !overlay) return;
    const video = node.querySelector("video.html5-main-video") as HTMLVideoElement | null;
    if (video) {
      try {
        if (!video.muted) { video.muted = true; mutedForAd = true; }
        if (video.playbackRate !== 16) { video.playbackRate = 16; rateForAd = true; }
  ...
  const onClassChange = (): void => {
    const node = player();
    if (!node) return;
    if (isAdShowing(node)) {
      handleAdFrame();
      if (skipTimer === undefined) skipTimer = window.setInterval(handleAdFrame, 400);
    } else {
      stopTimer();
...
function installDevtoolsBlock(): void {
...
function install(): void {
  if (window.__YOOD__) return;
  window.__YOOD__ = { invoke, scan };
  installDevtoolsBlock();
  installPlayerAdSkip();
  installAppearance();
  void invoke("get_settings").then((settings) => {
    const shortcut = (settings as { shortcut_download?: unknown }).shortcut_download;
  ...
  let scheduled = false;
  let navigationRetry: number | undefined;
  const schedule = (): void => { if (scheduled) return; scheduled = true; window.setTimeout(() => { scheduled = false; scan(); }, 180); };
  const scheduleForNavigation = (): void => {
    schedule();
    if (navigationRetry !== undefined) window.clearTimeout(navigationRetry);
  ...
  const observePage = (): void => {
    if (document.documentElement) {
      new MutationObserver((records) => {
        for (const record of records) {
          for (const node of record.addedNodes) {
            if (needsCardScan(node)) { schedule(); return; }
          }
        }
      }).observe(document.documentElement, { childList: true, subtree: true });
    }
  ...
  observePage();
  if (!document.documentElement) window.addEventListener("DOMContentLoaded", observePage, { once: true });
  document.addEventListener("keydown", (event) => {
    if (event.shiftKey && event.key.toLowerCase() === downloadShortcut && !/input|textarea|select/i.test((event.target as HTMLElement)?.tagName || "")) {
      event.preventDefault();
  ...
  if (document.readyState === "loading") window.addEventListener("DOMContentLoaded", scan, { once: true });
  else scan();
}
