//! Brave-first runtime (Linux): the `yood` binary launches Brave itself in
//! app mode and then runs headless (downloads, deep links, single instance)
//! with no WebKit window at all. No `./run.sh brave` needed for end users.
//!
...
//!
//! Launch flags mirror `linux/run.sh brave` — keep the two in sync.
...
fn bundled_or_system_brave() -> Option<PathBuf> {
    ...
    // Installed layout: `yood` next to `brave/brave` (see linux/scripts/install.sh).
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            // Tauri-bundle layout: resources keep their repo-relative shape
            // (binaries/linux-x86_64/brave) next to the executable; some
...
fn extension_dir() -> Option<PathBuf> {
    ...
    // install layout — AppImage, portable, dev — loads the current version.
...
fn seed_profile_prefs(profile: &std::path::Path) {
    seed_prefs_file(
        ...
        &[
            ...
            // while a fresh profile on another machine would not.
            // Seed it explicitly so this never depends on a
...
pub fn launch(app: &tauri::AppHandle, runtime: &RuntimeState) -> Result<(), String> {
    let brave = bundled_or_system_brave().ok_or_else(|| {
        "no Brave browser found (set YOOD_BRAVE_DIR, stage one with linux/scripts/fetch-brave.sh, or install brave-bin)".to_string()
    })?;
