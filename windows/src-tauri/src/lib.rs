    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            #[cfg(any(target_os = "linux", target_os = "windows"))]
            {
                ...
                // was launched from an AppImage (or a portable Windows folder)
                // that a user has not run through a desktop-integration tool.
                // On Windows this writes a per-user (HKCU) registration, no
                // admin needed. Failure here (for example xdg-mime being
                // unavailable) must not stop startup.
...
fn self_test() -> error::AppResult<()> {
    ...
    let rules: serde_json::Value =
        serde_json::from_str(include_str!("../../../shared/extension/yood/rules.json")).map_err(|_| {
